...
import com.cloudpos.demo.cert.util.ShellUtils.CommandResult;
...
public class PackageUtils {
    ...
    public static int installSilent(Context context, String filePath, boolean isUpdate) {
         ...
         **/
        String cmdTemp = "LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install ";//LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install -r 
        if(isUpdate){
            cmdTemp = cmdTemp + "-r ";
        }
        StringBuilder command = new StringBuilder().append(cmdTemp)
                                                   .append(filePath.replace(" ", "\\ "));
        CommandResult commandResult = ShellUtils.execCommand(command.toString(), !isSystemApplication(context), false);//TODO true
